    ...
    public class LocalRequestHandler : RequestHandler
    {
        ...
        public override void HandleRequest(object nullObj)
        {
            ...
            try
            {
                ...
                if (IsCacheable() && Proxy.ProxyCacheManager.IsCached(_rcRequest.RelCacheFileName))
                {
                    // Log query metric
                    Proxy.Logger.QueryMetric(Proxy.SessionManager.GetUserId(ClientIP),
                        true, RefererUri, RequestUri);
                    ...
                    if (Proxy.NetworkStatus != RCLocalProxy.NetworkStatusCode.Online
                        && Proxy.ProxyCacheManager.IsHTMLFile(_rcRequest.RelCacheFileName))
                    {
                        ...
                        GlobalCacheItem gci = _proxy.ProxyCacheManager.
                            GetGlobalCacheItemAsRequest(_rcRequest.RelCacheFileName);
                        if (gci == null)
                        {
